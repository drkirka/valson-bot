...
def normalize_class_name(value: str) -> str:
    """Keep class/profile search predictable across case and extra spaces."""
...
async def process_height(message: Message, state: FSMContext):
    ...
    try:
        height = int(clean_text(message.text))
    except ValueError:
...
async def process_contact(message: Message, state: FSMContext):
    language = await get_user_language(message.from_user.id)
    contact = clean_text(message.text)
...
async def menu_router(message: Message, state: FSMContext):
    ...
    if message.text == t(language, "delete_profile"):
        ...
        if not existing_profile:
            await message.answer(
                t(language, "no_profile_to_delete"),
                reply_markup=main_keyboard(language),
            )
            return
