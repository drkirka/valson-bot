...
def gender_keyboard(language: str) -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text=t(language, "gender_girl")), KeyboardButton(text=t(language, "gender_boy"))]],
        resize_keyboard=True,
        one_time_keyboard=True,
    )
...
def browse_keyboard(index: int, total: int, language: str) -> InlineKeyboardMarkup:
    ...
    nav = []
    if index > 0:
        nav.append(InlineKeyboardButton(text="⬅️", callback_data=f"page:{index - 1}"))
    if index < total - 1:
        nav.append(InlineKeyboardButton(text="➡️", callback_data=f"page:{index + 1}"))
    if nav:
        buttons.append(nav)
